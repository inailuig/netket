...
As the objects $\psi_{\bf{W}}$ is defined on an Hilbert space, the correct metric there is $ L_2 $ and not the Euclidean norm, we consider the distance defined as

This is the Quantum Geometric Tensor.
In NetKet you can obtain (a local approximation of) the quantum geometric tensor of a variational state by calling `variational_state.quantum_geometric_tensor())`.
...
The 3 implemnentations are:
 ...
 - {ref}`netket.optimizer.qgt.QGTOnTheFly`, which uses jax automatic differentiation through two `vjp` and one `jvp` product to compute the action of quantum geometric tensor on a vector and operates natively on PyTrees. This method shines if the parameters of your network are stored in a PyTree with few leaf nodes.
 - {ref}`netket.optimizer.qgt.QGTJacobianDense`, which precomputes the log derivatives ( $ O_k $ ) when it's constructed and converts it to a single dense array. If you have a high number of total parameters or many leaf nodes in your parameter PyTree, this implementation might perform better because everything is stored contiguously in memory. However, it has an high 'startup cost'. This method might not work and/or give wrong results if you mix real and complex parameters.
 - {ref}`netket.optimizer.qgt.QGTJacobianPyTree`, same as above, but the precomputed jacobian is not stored contiguously in memory but is stored as a PyTree. This will work better than `QGTJacobianDense` if there are few leaf nodes, but better than `QGTOnTheFly` if you use iterative solvers that perform many steps.
...
All the QGT implemnetation listed above have several options that can affect their performance. 
...
The [stochastic reconfiguration method](https://www.attaccalite.com/PhDThesis/html/node15.html) is a technique that makes use of the information encoded in the quantum geometric tensor described above to _precondition_ the gradient used in stochastic optimization, _improving_ the convergence rate to the ground state of an hamiltonian $ \hat{H} $.

We would like to underline that Stochastic Reconfiguration can be derived (and therefore thought of as) an imaginary time evolution. 
...
To use the Stochastic reconfiguration, you must simply provide it as a preconditioner to the VMC solver.
