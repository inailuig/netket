...
@dispatch
def random_state(hilb: Spin, key, batches: int):
    shape = (batches,)
...
def _random_states(hilb, key, shape):
        ...
        return (2 * rs - (n_states - 1)).astype(hilb.dtype)


@partial(jax.jit, static_argnames=('hilb', 'shape'))
def _random_states_with_constraint(hilb, rngkey, shape):
    ...
    if n_states == 2:
        ...
        nup = (N + m) // 2
        ndown = (N - m) // 2
        x = jnp.ones(shape+(nup+ndown,), dtype=hilb.dtype).at[..., -ndown:].set(-1)
        ...
        init = jnp.full(shape+(hilb.size,), -round(2 * hilb._s), dtype=hilb.dtype)
