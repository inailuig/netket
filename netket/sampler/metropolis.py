...
class MetropolisSamplerState(SamplerState):
    ...
    n_accepted_proc: jnp.ndarray = struct.field(
        default_factory=lambda: jnp.zeros((), dtype=jnp.int64) # TODO (c): initialize with shape σ.shape[0] to avoid recompilation
    )
    ...
    def n_accepted(self) -> int:
        ...
        res, _ = mpi.mpi_sum_jax(jax.jit(jnp.sum)(self.n_accepted_proc))  # jit sum for gda
        return res
...
class MetropolisSampler(Sampler):
    ...
    def _reset(sampler, machine, parameters, state):
        ...
        return state.replace(
            σ=σ, rng=new_rng, rule_state=rule_state, n_steps_proc=0, n_accepted_proc=jnp.zeros(σ.shape[0], dtype=jnp.int64)
        )
