...
def _apply_term_scan_math(x, weight, sites, daggers, unroll=1):
    ...
    if not (jnp.issubdtype(x.dtype, jnp.signedinteger) or jnp.issubdtype(x.dtype, jnp.floating)):
        raise ValueError(f"incompatible dtype for x, expect a type with sign but got {x.dtype}")
    ...
    def f(carry, xs):
        ...
        mask_all_up_to_site = (jnp.arange(n_orbitals, dtype=sites.dtype) < site).astype(x_.dtype)
